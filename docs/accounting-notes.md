...
## Formulas
- **Account Balance**: cash after fills and fees. Buying reduces cash by `price × qty + fee`. Selling long increases cash by `price × qty − fee`. Short sales do not credit proceeds; cash only pays fees while margin tracks the short exposure.
- **Equity**: `cash + unrealized P&L`.
- **Unrealized P&L**:
  - Long: `qty × (last_price − average_price)`.
  - Short: `|qty| × (average_price − last_price)`.
- **Realized P&L**:
  - Closing long: `(sell_price − avg_long_price) × closed_qty − close_fee_share − proportional_entry_fees`.
  - Covering short: `(avg_short_price − buy_price) × closed_qty − close_fee_share − proportional_entry_fees`.
...
- Entry fees for open quantities are tracked so that proportional shares can be deducted from realized P&L when positions are reduced.
